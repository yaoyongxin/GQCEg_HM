...
def modify_sparse_pauli_op(sp_op, permutation):
    """
    Modify a SparsePauliOp based on a given qubit layout.

    Parameters:
        sp_op (SparsePauliOp): The SparsePauliOp to modify.
        permutation (list): logical qubits to physical qubits.

    Returns:
        SparsePauliOp: The modified SparsePauliOp with reordered qubits.
    """

    # Reorder the Pauli strings manually
    reordered_pauli_strings = []
    for pauli in sp_op.paulis:
        pauli_str = pauli.to_label()[::-1]
        reordered_pauli = ''.join(pauli_str[i] for i in permutation)
        reordered_pauli_strings.append(reordered_pauli[::-1])

    # Create a new SparsePauliOp with reordered Pauli strings and the same coefficients
    sp_op_new = SparsePauliOp.from_list(list(zip(reordered_pauli_strings, sp_op.coeffs)))

    # print(f"{sp_op} vs {sp_op_new}")
    return sp_op_new
...
if False:
    ...
    for i in range(2):
        job = function.run(
                ...
                pubs=[pub],
                options = {"mitigation_level": 2, "default_precision": 0.01},
                )
...
if False:
    ...
    qctrl_estimator_job = perf_mgmt.run(
        ...
        # options={"default_shots": 2**16}, not significant
    )
    print(qctrl_estimator_job)
    print(qctrl_estimator_job.status())

if False:
    # algorithmiq: to linear connectivity
    from qiskit import transpile
    from qiskit.transpiler import CouplingMap

    # Define a linear connectivity coupling map
    # linear_coupling = CouplingMap([[i, i+1] for i in range(qc.num_qubits-1)]
    #         + [[i+1, i] for i in range(qc.num_qubits-1)])

    linear_coupling = CouplingMap([[i, i+1] for i in range(qc.num_qubits-1)])

    # Transpile the circuit to match the linear connectivity
    # qc = transpile(qc, coupling_map=linear_coupling, optimization_level=3)
    qc = transpile(qc, coupling_map=linear_coupling, optimization_level=1)
    print(qc)
    print("\nTranspiled Circuit for Linear Connectivity:")
    print(f"linear circuit depth: {qc.depth()}")
    lq_to_pq = qc.layout.final_index_layout()
    pq_to_lq = np.argsort(lq_to_pq)
    print(f"physical to logical qubits: {pq_to_lq}")
    obs_list_qiskit = [modify_sparse_pauli_op(op, pq_to_lq) for op in obs_list_qiskit]
    ...
    print("exact expvals from linear circuit:")
    for ob, val, vadd in zip(obs_list_qiskit, result.data.evs, const_list):
        # print(f"{ob}: {val+vadd}, {val}")
        print(f"{val+vadd:10.6f}, {val:10.6f}")
...
if False:
    ...
    service = QiskitRuntimeService()
    backend_name = service.least_busy(operational=True, simulator=False).name
    print(f"backend_name: {backend_name}")

    qesem_estimator_job = qesem_function.run(
        pubs=[(qc, obs_list_qiskit)],
        instance=hub + "/" + group + "/" + project,
        backend_name=backend_name,
        options={"estimate_time_only": "empirical"},
    )
    print(qesem_estimator_job)
    print(qesem_estimator_job.status())
    print(qesem_estimator_job.result())
