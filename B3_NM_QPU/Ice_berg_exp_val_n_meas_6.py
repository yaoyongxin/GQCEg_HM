...
for s in [0.1,0.2,0.5,1.0,2.0]: 
    ...
    qc_noise_nm=add_noise(qc_test,p_init,p2,p1,p_meas)
    count_dict={}
    for i,Pstr in enumerate(P_str):
        qc_noise=qc_noise_nm.copy()
        if Pstr == 'IIIIIIII':
            continue
...
#print('Unencode:',exp_vals)
    
with open('Unenc_u_30_m.pickle', 'wb') as handle:
...
def measure_stb(qc,p_init,p2,p1,p_meas,N_mea):
    ...
    for i in range(size-2):
        if i==1 or i== size-3:
            qc.cx(qc.qregs[0][-1],qc.qregs[0][-2])
        qc.cx(qc.qregs[0][-1],qc.qregs[0][i])
        qc.append(Noise_Model["Two_q_err"],[qc.qregs[0][i],qc.qregs[0][-1]])
    ...
    qc.append(Noise_Model["meas_error"],[qc.qregs[0][-1]])
    qc.measure(qc.qregs[0][-1],qc.cregs[0][2*N_mea-2])
    qc.append(Noise_Model["meas_error"],[qc.qregs[0][-2]])
    qc.measure(qc.qregs[0][-2],qc.cregs[0][2*N_mea-1])
    ...
    for i in range(size-2):
        if i==1 or i== size-3:
            qc.cx(qc.qregs[0][-1],qc.qregs[0][-2])
        qc.cx(qc.qregs[0][i],qc.qregs[0][-2])
        qc.append(Noise_Model["Two_q_err"],[qc.qregs[0][i],qc.qregs[0][-1]])
        #qc.append(Noise_Model["Two_q_err"],[qc.qregs[0][-1]])
        #qc.append(Noise_Model["Two_q_err"],[qc.qregs[0][i]])
    ...
    qc.append(Noise_Model["meas_error"],[qc.qregs[0][-1]])
    qc.measure(qc.qregs[0][-1],qc.cregs[0][2*N_mea])
    qc.append(Noise_Model["meas_error"],[qc.qregs[0][-2]])
    qc.measure(qc.qregs[0][-2],qc.cregs[0][2*N_mea+1])
    
N_mea=4
model = QiskitCircuitGeneratorAVQITE1(ansatz_filename='ansatz_inp.pkle', incar_filename='incar')
qc_ice = model.construct_ansatz_qc(N_mea,decompose_rzz=False)

qc_ice=add_meas(qc_ice,N_mea)
...
enc_dict={}
pass_dict={}
#sh_b=[1.07862,1.16141,1.42801,1.92618,2.8824] # for # of mes=1 and 50
#sh_b=[1.08586,1.17807,1.49225,2.16032,4.07483] # for # of mes=2 and 50
#sh_b=[1.09265,1.19324,1.5451,2.33833,4.96494] # for # of mes=3 and 50
#sh_b=[1.09941,1.2087,1.59884,2.51064,5.82761] # for # of mes=4 and 50

#sh_b=[1.04502,1.09129,1.23766,1.50258,2.08572] # for # of mes=1 and 30
#sh_b=[1.05178,1.10574,1.28222,1.62452,2.52601] # for # of mes=2 and 30
#sh_b=[1.05828,1.11994,1.32344,1.73539,2.92438] # for # of mes=3 and 30
sh_b=[1.06465,1.13376,1.36593,1.85494,3.34376] # for # of mes=4 and 30
ith=0
for s in [0.1,0.2,0.5,1.0,2.0]:
    count_dict={}
    p=3e-3*s
    ...
    qc_noise_nm=add_noise(qc_ice,p_init,p2,p1,p_meas)
    #measure_stb(qc_noise,p_init,p2,p1,p_meas,N_mea)
    for i,Pstr in enumerate(P_str):
        qc_noise=qc_noise_nm.copy()
        ...
        for j in range(qc_noise.qregs[0]._size-2):
            qc_noise.append(Noise_Model["meas_error"],[qc_noise.qregs[0][j]])
        for j in range(qc_noise.qregs[0]._size-2):
            qc_noise.measure(qc_noise.qregs[0][j],qc_noise.cregs[0][2*N_mea+j])
    ...
    for Pstr in P_str:
        ...
        for k, w in enumerate(Pstr):
            if w!='I':
                pos_lst.append(k+2*N_mea)
        val=0
        num=0
        for key in count_dict[Pstr].keys():
            new_key = np.array(list(map(int,list(key[::-1]))))
            if np.sum(new_key[:2*N_mea])==0:
    ...
    ith+=1
#print('Encoded:',exp_vals)

with open('enc_val_4_u_30_ft.pickle', 'wb') as handle:
    pickle.dump(enc_dict, handle, protocol=pickle.HIGHEST_PROTOCOL)
    
with open('enc_suc_4_u_30_ft.pickle', 'wb') as handle:
...
#print('\n')
#print('Exact:',np.real(new_expval))
with open('EXT_expvals_u_30_m.npy', 'wb') as f:
