...
if True:
    ...
    if False:
        ...
        for mc in measurement_setup.measurement_circs:
            ...
            measurement_circuit_ref = qnx.circuits.upload(
                circuit=c,
                name=f"state prep + meas circuit_L50 from {datetime.now()}",
                project = project,
            )
    ...
    if False:
        ...
        circs_c = qnx.start_compile_job(
            ...
            name=f"Compile Job_L50 from {datetime.now()}",
        ...
        qnx.filesystem.save(
                ref=circs_c,
                path=Path.cwd() / "compile_ref" / "0_3",
                mkdir=True,
                )
        print("start_compile_job submitted")
        quit()

    if True:
        # retrieve compile circuits
        compile_ref = qnx.filesystem.load(
                path=Path.cwd() / "compile_ref" / "0_3_H1_1LE",
                )
        ...
        job_ref = qnx.start_execute_job(
                name=f"execute_job_L50_{datetime.now()}",
                circuits=circs_c,
        ...
        qnx.filesystem.save(
                ref=job_ref,
                path=Path.cwd() / "exec_ref" / "0_3_sv_noiseless",
                mkdir=True,
                )
        print("start_execute_job submitted")
        quit()
    ...
    for i, op in enumerate(obs_list_pytket):
        ...
        print(f"nexus: {i} {res.real:10.6f}")
